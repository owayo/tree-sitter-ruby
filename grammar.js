	...
	rules: {
		program: ($) =>
			seq(
				...
				optional(choice(seq(/__END__/, $.uninterpreted))),
			),
				...
				choice(
					...
					seq(
						optional(
							field(
								"parameters",
								alias($.bare_parameters, $.method_parameters),
							),
						),
						$._terminator,
						optional(field("body", $.body_statement)),
		...
		rescue_modifier_arg: ($) =>
			prec(
				PREC.RESCUE,
				seq(field("body", $._arg), "rescue", field("handler", $._arg)),
			),
		...
		splat_parameter: ($) =>
			prec.right(-2, seq("*", field("name", optional($.identifier)))),
		hash_splat_parameter: ($) =>
			seq("**", field("name", optional($.identifier))),
		hash_splat_nil: (_) => seq("**", "nil"),
		block_parameter: ($) => seq("&", field("name", optional($.identifier))),
		keyword_parameter: ($) =>
			prec.right(
				PREC.BITWISE_OR + 1,
				seq(
			...
			prec.right(
				choice(
					...
					seq(
						$._pattern_expr,
						",",
						choice($._pattern_expr, $._array_pattern_n),
					),
		...
		as_pattern: ($) =>
			seq(field("value", $._pattern_expr), "=>", field("name", $.identifier)),

		_pattern_expr_alt: ($) =>
			choice($.alternative_pattern, $._pattern_expr_basic),
		...
		_hash_pattern_any_rest: ($) =>
			choice($.hash_splat_parameter, $.hash_splat_nil),
		...
		_keyword_variable: ($) =>
			prec.right(
				-1,
				choice($.nil, $.self, $.true, $.false, $.line, $.file, $.encoding),
			),
		...
		variable_reference_pattern: ($) =>
			seq("^", field("name", choice($.identifier, $._nonlocal_variable))),
		...
		begin: ($) =>
			seq("begin", optional($._terminator), optional($._body_statement), "end"),
		...
		_body_statement: ($) =>
			choice(
				...
				seq(
					optional($._statements),
					repeat1(choice($.rescue, $.else, $.ensure)),
				),
			),
		...
		forward_argument: (_) => "...",
		splat_argument: ($) =>
			prec.right(seq(alias($._splat_star, "*"), optional($._arg))),
		hash_splat_argument: ($) =>
			prec.right(seq(alias($._hash_splat_star_star, "**"), optional($._arg))),
		block_argument: ($) =>
			prec.right(seq(alias($._block_ampersand, "&"), optional($._arg))),
			...
			prec.right(
				...
				choice(
					seq(
						...
						field(
							"right",
							choice($._arg_rhs, $.splat_argument, $.right_assignment_list),
						),
					),
				),
			),
		...
		_variable: ($) =>
			prec.right(
				choice($.self, $.super, $._nonlocal_variable, $.identifier, $.constant),
			),
		...
		constant_suffix: ($) =>
			choice(token(seq(/[A-Z]/, IDENTIFIER_CHARS, /[?]/)), $._constant_suffix),
		identifier: (_) => token(seq(LOWER_ALPHA_CHAR, IDENTIFIER_CHARS)),
		identifier_suffix: ($) =>
			choice(
				token(seq(LOWER_ALPHA_CHAR, IDENTIFIER_CHARS, /[?]/)),
				$._identifier_suffix,
			),
